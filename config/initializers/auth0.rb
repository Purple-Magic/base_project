...
def auth0_skip_configuration_error?
  ENV["SECRET_KEY_BASE_DUMMY"].present? || ARGV.include?("assets:precompile")
end
...
if auth0_skip_configuration_error?
  Rails.configuration.x.auth0.domain = nil
  Rails.configuration.x.auth0.client_id = nil
  Rails.configuration.x.auth0.client_secret = nil
  Rails.logger.warn("Skipping Auth0 configuration during assets precompile.")
else
